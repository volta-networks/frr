...
/* LDP IGP Sync not achieved */
    ...
    {LDP_SYNC_STA_NOT_ACH,	LDP_SYNC_EVT_CONFIG_LDP_OFF,	LDP_SYNC_ACT_CONFIG_LDP_OFF,	LDP_SYNC_STA_NOT_ACH},
    {LDP_SYNC_STA_NOT_ACH,	LDP_SYNC_EVT_IFACE_SHUTDOWN, 	LDP_SYNC_ACT_IFACE_SHUTDOWN,	LDP_SYNC_STA_NOT_ACH},
...
ldp_sync_fsm_nbr_event(struct nbr *nbr, enum ldp_sync_event event)
{
	...
	RB_FOREACH(adj, nbr_adj_head, &nbr->adj_tree) {
		...
		if (event == LDP_SYNC_EVT_SESSION_CLOSE && oper_nbr_count > 0) {
			 ...
			 */
#ifdef LDP_SYNC_FSM_DEBUG_VERBOSE
			debug_evt_ldp_sync("%s: Ignoring session close; "
				"lsr-id %s, event %s, interface=%s (%d), "
				"state=%s, oper_nbr_count=%d",
